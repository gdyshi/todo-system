...
def upgrade() -> None:
    ...
    op.add_column(
        "tasks",
        sa.Column("parent_id", sa.Integer(), sa.ForeignKey("tasks.id", ondelete="CASCADE"), nullable=True),
    )
    # Change priority from String to Integer
    op.alter_column("tasks", "priority", existing_type=sa.String(), type_=sa.Integer(), nullable=True)
    # Add due_time column
    ...
    op.alter_column("tasks", "description", existing_type=sa.String(), type_=sa.Text(), nullable=True)


def downgrade() -> None:
    op.alter_column("tasks", "description", existing_type=sa.Text(), type_=sa.String(), nullable=True)
    op.drop_column("tasks", "reminder_sent")
    op.drop_column("tasks", "location")
    op.drop_column("tasks", "due_time")
    op.alter_column("tasks", "priority", existing_type=sa.Integer(), type_=sa.String(), nullable=True)
